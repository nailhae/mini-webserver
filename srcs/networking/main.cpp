...
{
	std::cout << Colors::BoldGreen << "close client:" << \
	udata.GetFd() << Colors::Reset << std::endl;
	delete &udata;
...
int main()
{
	...
	if (kq == -1)
	{
		std::cerr << Colors::RedString("Error: Open Error") << std::endl;
		return (1);
	}
	...
	while (1)
	{
		occurEventNum = kevent(kq, changeList.getKeventVector().data(), changeList.getSize(), eventList, MAX_KEVENTS, NULL);
		...
		for (int i = 0; i < occurEventNum; i++)
		{
			...
			else if (eventList[i].filter & EVFILT_READ)
			{
				...
				if (len == 0 || (eventList[i].fflags & EV_EOF) == true) // close request -> write 이벤트도 등록해야 한다.
